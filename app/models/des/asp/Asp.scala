...
object Asp {
  ...
  implicit def conv(asp: models.fe.asp.Asp) : Asp = {
    asp.otherBusinessTaxMatters match {
      case Some(OtherBusinessTaxMattersYes) => Asp(true, None)
      case Some(OtherBusinessTaxMattersNo) => Asp(false, None)
      case _ => Asp(false, None)
    }
  }
}
