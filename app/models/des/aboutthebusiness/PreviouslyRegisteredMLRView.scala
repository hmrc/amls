...
case class PreviouslyRegisteredMLRView(
  ...
  implicit def convert(businessDetails: BusinessDetails): Option[PreviouslyRegisteredMLRView] =
    businessDetails.previouslyRegistered match {
      case x: PreviouslyRegisteredYes if x.value.getOrElse("").length == 15 =>
        Some(PreviouslyRegisteredMLRView(false, None, true, x.value))
      case x: PreviouslyRegisteredYes if x.value.getOrElse("").length == 8  =>
        Some(PreviouslyRegisteredMLRView(true, x.value, false, None))
