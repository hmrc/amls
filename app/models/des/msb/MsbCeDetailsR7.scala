...
case class MsbCeDetailsR7(
  dealInPhysCurrencies: Option[Boolean] = None,
...
object MsbCeDetailsR7 {
  ...
  implicit def conv(msb: models.fe.moneyservicebusiness.MoneyServiceBusiness): Option[MsbCeDetailsR7] = {
    ...
    val dealInPhysCurrencies = msb.whichCurrencies.fold[Option[Boolean]](Some(false))(wc =>
    ...
    Some(
      MsbCeDetailsR7(
        dealInPhysCurrencies,
        if (dealInPhysCurrencies.isEmpty || dealInPhysCurrencies.get) {
          msb
        } else None,
  ...
  implicit def convertFromOldModel(msbceOpt: Option[MsbCeDetails]): Option[MsbCeDetailsR7] =
    msbceOpt map { msbce =>
      MsbCeDetailsR7(
        msbce.dealInPhysCurrencies,
        Some(
          CurrencySourcesR7(
            msbce.currencySources.bankDetails,
            msbce.currencySources.currencyWholesalerDetails,
