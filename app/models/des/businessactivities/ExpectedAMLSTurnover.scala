...
case class OtherBusinessActivities(
...
case class ExpectedAMLSTurnover(
  mlrActivityTurnover: Option[String] = None,
  ...
  implicit def convert(bact: models.fe.businessactivities.BusinessActivities): Option[ExpectedAMLSTurnover] =
    bact.involvedInOther match {
      case Some(InvolvedInOtherNo)     =>
        bact.expectedAMLSTurnover match {
          case Some(aMLSTurnover) => Some(ExpectedAMLSTurnover(convertAMLSTurnover(aMLSTurnover)))
          case None               => None
        }
      case Some(InvolvedInOtherYes(x)) =>
        Some(
          ExpectedAMLSTurnover(otherBusActivitiesCarriedOut =
            Some(
              OtherBusinessActivities(
                x,
                convertTurnover(bact.expectedBusinessTurnover),
                bact.expectedAMLSTurnover.fold("")(x => convertAMLSTurnover(x).getOrElse(""))
              )
            )
          )
        )
