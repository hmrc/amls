...
object PositionInBusiness {
  ...
  private def getPositionAsflags(positions: Positions) =
    positions.positions.foldLeft((false, false, false, false, false, false, false, false)) { (pos, p) =>
      p match {
  ...
  implicit def convPositions(
    positions: Positions,
    bm: fe.businessmatching.BusinessMatching
  ): Option[PositionInBusiness] = {
    val (beneficialOwner, director, _, nominatedOfficer, partner, soleProprietor, designatedMember, other) =
    ...
    bm.reviewDetails.businessType match {
      case BusinessType.SoleProprietor                                                         =>
        Some(
          PositionInBusiness(Some(SoleProprietor(soleProprietor, nominatedOfficer, assignOther, otherVal)), None, None)
        )
      case BusinessType.Partnership                                                            =>
        Some(PositionInBusiness(None, Some(Partnership(partner, nominatedOfficer, assignOther, otherVal)), None))
