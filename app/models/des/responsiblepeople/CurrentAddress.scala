...
object CurrentAddress {
  ...
  private def removeAmpersands(currentAddress: CurrentAddress): CurrentAddress = {
    ...
    currentAddress.copy(address =
      currentAddress.address.copy(
        addressLine1 = removeFromLine(Some(currentAddress.address.addressLine1)).getOrElse(""),
          ...
          removeAmpersands(
            CurrentAddress(
              AddressWithChangeDate(
                ...
                uk.addressLine4,
                "GB",
                convertEmptyOrInvalidToNone(uk.postCode),
                addrHistory.dateOfChange
              )
            )
          )
        )
