...
trait TradingPremisesUpdateHelper {
  ...
  private def tradingPremisesWithStatus(
    desTradingPremises: TradingPremises,
    viewTradingPremises: TradingPremises
  ): TradingPremises = {
    ...
    desTradingPremises.copy(
      agentBusinessPremises = updatedAgentBusinessPremises,
      ownBusinessPremises = updatedOwnBusinessPremises
    )
  }

  private def getAgentDetailsWithStatus(
    desTradingPremises: TradingPremises,
    viewTradingPremises: TradingPremises
  ): Option[AgentBusinessPremises] = {
    val (agentWithLineIds, agentWithoutLineIds) = desTradingPremises.agentBusinessPremises match {
      case Some(agentBusinessPremises) =>
        desTradingPremises.agentBusinessPremises.get.agentDetails
          .fold[(Seq[AgentDetails], Seq[AgentDetails])]((Seq.empty, Seq.empty))(agent =>
            agent.partition(_.lineId.isDefined)
          )
      case None                        => (Nil, Nil)
    }
    ...
    desTradingPremises.agentBusinessPremises.map { agentBusinessPremises =>
      agentBusinessPremises.copy(agentDetails = Some(addAgentList))
    }
  }

  private def getOwnPremisesWithStatus(
    desTradingPremises: TradingPremises,
    viewTradingPremises: TradingPremises
  ): Option[OwnBusinessPremises] = {

    val (ownWithLineIds, ownWithoutLineIds) = desTradingPremises.ownBusinessPremises match {
      case Some(ownBusinessPremises) =>
        ownBusinessPremises.ownBusinessPremisesDetails
          .fold[(Seq[OwnBusinessPremisesDetails], Seq[OwnBusinessPremisesDetails])]((Seq.empty, Seq.empty))(own =>
            own.partition(_.lineId.isDefined)
          )
      case None                      => (Nil, Nil)
    }
    ...
    desTradingPremises.ownBusinessPremises.map { ownBusinessPremises =>
      ownBusinessPremises.copy(ownBusinessPremisesDetails = Some(addOwnList))
    }
  }

  private def updateOwnPremisesStatus(
    ownDetails: OwnBusinessPremisesDetails,
    viewTradingPremises: TradingPremises
  ...
  private def updateOwnPremisesStatus(ownDetails: OwnBusinessPremisesDetails, viewOwnDtls: OwnBusinessPremisesDetails) =
    ownDetails.status match {
      ...
      case _                             =>
        ownDetails.equals(viewOwnDtls) match {
          case true  => StatusConstants.Unchanged
          case false => StatusConstants.Updated
        }
    }

  private def updateOwnPremisesStartDateFlag(
    ownDetails: OwnBusinessPremisesDetails,
    viewOwnDtls: OwnBusinessPremisesDetails
  ) =
    Some((!ownDetails.status.contains(StatusConstants.Deleted)) && !ownDetails.startDate.equals(viewOwnDtls.startDate))
  ...
  private def updateAgentDetailsStatus(agentDetails: AgentDetails, viewAgent: AgentDetails) =
    agentDetails.status match {
      case Some(StatusConstants.Deleted) => StatusConstants.Deleted
      case _                             =>
        agentDetails.equals(viewAgent) match {
          case true  => StatusConstants.Unchanged
          case false => StatusConstants.Updated
        }
    }
