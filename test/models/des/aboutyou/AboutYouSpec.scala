...
class AboutYouRelease7Spec extends PlaySpec {
  ...
  "AboutYouRelease7" when {
    "given the release 7 json" must {
      "be serialisable for roleWithinBusiness" in {
    ...
    "given the PRE release 7 json" must {
      "be able to read for roleWithinBusiness" in {
      ...
      "be able to read for roleForTheBusiness" in {
      ...
      // does it really need to write??  only the reads should be different?
    }
