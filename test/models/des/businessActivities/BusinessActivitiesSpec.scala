...
class BusinessActivitiesSpec extends PlaySpec {
  ...
  "BusinessActivities" must {
    ...
    val eabServiceModel   = Some(EabServices(true, false, false, false, false, false, false, false, false))
    ...
    "serialise business activities model for eabServicesCarriedOut " in {
      BusinessActivities.format.writes(model) must be(
        Json.obj(
          ...
          "tcspServicesforRegOffBusinessAddrVirtualOff" -> Json.obj(
            "callHandling"         -> true,
            "emailHandling"        -> true,
            "emailServer"          -> true,
          ...
          "ampServicesCarriedOut"                       -> Json.obj(
            ...
            "privateDealer" -> true,
            "intermediary"  -> true,
            "other"         -> Json.obj("otherAnswer" -> true, "specifyOther" -> "Another service")
          )
        )
      )
    }
    ...
    val activitiesModel               = BusinessActivitiesAll(
      None,
      None,
      Some(false),
      activityDetails,
    ...
    val allmodel = BusinessActivities(
      bmMlrActivities,
      None,
      hvdGoodsSold,
      hvdAlcoholTobacco,
      ...
      eabServiceModel,
      ampServiceModel,
      Some(activitiesModel)
    )

    "serialise business activities model for all" in {
      BusinessActivities.format.writes(allmodel) must be(
        Json.obj(
          "mlrActivitiesAppliedFor"                     -> Json.obj(
            "msb"     -> true,
            "hvd"     -> false,
          ...
          "hvdGoodsSold"                                -> Json.obj(
            ...
            "howGoodsAreSold"    -> Json.obj("retail" -> true, "wholesale" -> true, "auction" -> true)
          ),
    ...
    "successfully compare api5 model with api6 model1" in {
      val viewModel = BusinessActivities(
        bmMlrActivities,
        None,
        hvdGoodsSold,
        hvdAlcoholTobacco,
