...
class SubscriptionRequestSpec extends PlaySpec with MockitoSugar with GuiceOneAppPerTest {
  ...
  /*
should be: Sole Proprietor,None,Some(1970-01-01)
PreviousNameDetails(true,Some(PersonName(Some(fname),Some(mname),Some(lname))),Some(1990-02-24),false))))
SubscriptionRequest(1234,BusinessDetails(SoleProprietor,None,None),BusinessContactDetails(Address(line1,line2,Some(some street),Some(some city),GB,Some(EE1 1EE),None),true,Some(AlternativeAddress(kap,Trading,Address(Park,lane,Some(Street),Some(city),GB,Some(EE1 1EE),None))),019212323222323222323222323222,abc@hotmail.co.uk),Some(PreviouslyRegisteredMLR(true,Some(12345678),false,None)),Some(VATRegistration(true,Some(123456789))),Some(CorporationTaxRegisteredCbUbLlp(true,Some(1234567890))),BusinessActivities(Some(MlrActivitiesAppliedFor(true,true,true,false,false,false,false,false)),Some(MsbServicesCarriedOut(true,true,false,true,false)),Some(HvdGoodsSold(true,true,true,true,true,false,false,false,false,false,false,true,Some(Details),Some(HowGoodsAreSold(true,false,true)))),Some(HvdAlcoholTobacco(true)),Some(AspServicesOffered(true,false,false,true,true)),Some(TcspServicesOffered(true,true,true,true,true)),Some(ServicesforRegOff(true,false,false,false,false,false,false,true,Some(other service))),Some(EabServices(false,false,true,false,true,false,false,false,false,Some(true))),Some(AmpServices(true,true,true,true,AmpServicesOther(true,Some(Another service)))),Some(BusinessActivitiesAll(None,Some(2001-01-01),false,BusinessActivityDetails(true,Some(ExpectedAMLSTurnover(Some(£0-£15k),None))),Some(FranchiseDetails(true,Some(List(Name)))),Some(10),Some(5),NonUkResidentCustDetails(true,Some(List(GB, AB))),AuditableRecordsDetails(Yes,Some(TransactionRecordingMethod(true,true,true,Some(value)))),true,true,Some(FormalRiskAssessmentDetails(true,Some(RiskAssessmentFormat(true,false)))),Some(MlrAdvisor(true,Some(MlrAdvisorDetails(Some(AdvisorNameAddress(Name,Some(TradingName),Address(Line1,Line2,Some(Line3),Some(Line4),GB,Some(AA1 1AA),None))),true,None))))))),TradingPremises(Some(OwnBusinessPremises(true,Some(List(OwnBusinessPremisesDetails(Some(string),Address(string,string,Some(string),Some(string),GB,Some(AA1 1AA),None),false,Msb(true,true,true,true,false),Hvd(true),Asp(false),Tcsp(true),Eab(false),Bpsp(false),Tditpsp(false),Amp(true),2010-01-01,None,None,None,None,false,None))))),Some(AgentBusinessPremises(true,Some(List(AgentDetails(Sole Proprietor,None,Some(1970-01-01),Some(entity name),AgentPremises(string,Address(string,string,Some(string),Some(string),GB,Some(AA1 1AA),None),true,Msb(false,false,false,false,false),Hvd(false),Asp(false),Tcsp(false),Eab(true),Bpsp(true),Tditpsp(false),Amp(true),Some(2008-01-01),None),None,false,None,None,None,None,None,None)))))),BankDetails(3,Some(List(BankAccount(Personal account,Personal,true,ukAccount(112233,12345678)), BankAccount(Business account,This business's,false,AccountNumber(12345678)), BankAccount(Another Business account,Another business's,false,IBANNumber(12345678))))),Some(MoneyServiceBusiness(Some(MsbAllDetails(Some(£15k-50k),true,Some(CountriesList(List(GB))),true)),Some(MsbMtDetails(true,Some(123456),IpspServicesDetails(true,Some(List(IpspDetails(name,123456789123456)))),true,Some(12345678963),Some(CountriesList(List(GB))),Some(CountriesList(List(LA, LV))),None)),Some(MsbCeDetailsR7(Some(true),Some(CurrencySourcesR7(Some(MSBBankDetails(true,Some(List(Bank names)))),Some(CurrencyWholesalerDetails(true,Some(List(wholesaler names)))),true)),12345678963,Some(CurrSupplyToCust(List(USD, MNO, PQR))))),None)),Some(Hvd(true,Some(1978-02-15),false,true,Some(40),Some(HvdFromUnseenCustDetails(true,Some(ReceiptMethods(true,true,true,Some(foo))))))),Some(Asp(true,None)),Some(AspOrTcsp(Some(SupervisionDetails(true,Some(SupervisorDetails(Company A,1993-08-25,1999-08-25,false,Ending reason)))),Some(ProfessionalBodyDetails(true,Some(details),Some(ProfessionalBodyDesMember(true,Some(MemberOfProfessionalBody(true,true,false,false,false,false,false,false,false,false,false,false,false,true,Some(test))))))))),Some(TcspAll(true,Some(12345678))),Some(TcspTrustCompFormationAgt(true,true)),Some(EabAll(true,Some(Details),false,None)),Some(EabResdEstAgncy(true,Some(Property Redress Scheme))),Some(Amp(TransactionsAccptOvrThrshld(true,Some(2019-09-19 16:58:06.259Z)),true,60)),Some(List(ResponsiblePersons(Some(NameDetails(PersonName(Some(name),Some(some),Some(surname)),Some(OthrNamesOrAliasesDetails(true,Some(List(Doc)))),Some(PreviousNameDetails(true,Some(PersonName(Some(fname),Some(mname),Some(lname))),Some(1990-02-24),false)))),Some(NationalityDetails(true,Some(IdDetail(Some(UkResident(nino)),None,Some(1970-01-01))),Some(GB),Some(GB))),Some(ContactCommDetails(test@test.com,07000001122,None)),Some(CurrentAddress(AddressWithChangeDate(ccLine 1,ccLine 2,None,None,GB,Some(AA1 1AA),None))),Some(0-6 months),Some(AddressUnderThreeYears(Address(Line 1,Line 2,None,None,GB,Some(BB1 1BB)))),Some(7-12 months),Some(AddressUnderThreeYears(Address(e Line 1,e Line 2,Some(e Line 3),Some(e Line 4),GB,Some(CC1 1CC)))),Some(1-3 years),Some(PositionInBusiness(Some(SoleProprietor(true,true,Some(false),None)),None,None)),Some(RegDetails(false,None,true,Some(0123456789))),true,Some(Some training),true,Some(test),Some(2022-07-07),false,None,Some(false),Some(true),RPExtra(None,None,None,None,None,None,None,None,None)))),AboutYouRelease7(Some(IndividualDetails(fName,None,lName)),true,Some(RolesWithinBusiness(false,false,false,false,false,false,false,true,Some(Agent))),Some(RoleForTheBusiness(false,true,Some(Agent)))),Declaration(true),Some(LettingAgents(Some(true))))
SubscriptionRequest(1234,BusinessDetails(SoleProprietor,None,None),BusinessContactDetails(Address(line1,line2,Some(some street),Some(some city),GB,Some(EE1 1EE),None),true,Some(AlternativeAddress(kap,Trading,Address(Park,lane,Some(Street),Some(city),GB,Some(EE1 1EE),None))),019212323222323222323222323222,abc@hotmail.co.uk),Some(PreviouslyRegisteredMLR(true,Some(12345678),false,None)),Some(VATRegistration(true,Some(123456789))),Some(CorporationTaxRegisteredCbUbLlp(true,Some(1234567890))),BusinessActivities(Some(MlrActivitiesAppliedFor(true,true,true,false,false,false,false,false)),Some(MsbServicesCarriedOut(true,true,false,true,false)),Some(HvdGoodsSold(true,true,true,true,true,false,false,false,false,false,false,true,Some(Details),Some(HowGoodsAreSold(true,false,true)))),Some(HvdAlcoholTobacco(true)),Some(AspServicesOffered(true,false,false,true,true)),Some(TcspServicesOffered(true,true,true,true,true)),Some(ServicesforRegOff(true,false,false,false,false,false,false,true,Some(other service))),Some(EabServices(false,false,true,false,true,false,false,false,false,Some(true))),Some(AmpServices(true,true,true,true,AmpServicesOther(true,Some(Another service)))),Some(BusinessActivitiesAll(None,Some(2001-01-01),false,BusinessActivityDetails(true,Some(ExpectedAMLSTurnover(Some(£0-£15k),None))),Some(FranchiseDetails(true,Some(List(Name)))),Some(10),Some(5),NonUkResidentCustDetails(true,Some(List(GB, AB))),AuditableRecordsDetails(Yes,Some(TransactionRecordingMethod(true,true,true,Some(value)))),true,true,Some(FormalRiskAssessmentDetails(true,Some(RiskAssessmentFormat(true,false)))),Some(MlrAdvisor(true,Some(MlrAdvisorDetails(Some(AdvisorNameAddress(Name,Some(TradingName),Address(Line1,Line2,Some(Line3),Some(Line4),GB,Some(AA1 1AA),None))),true,None))))))),TradingPremises(Some(OwnBusinessPremises(true,Some(List(OwnBusinessPremisesDetails(Some(string),Address(string,string,Some(string),Some(string),GB,Some(AA1 1AA),None),false,Msb(true,true,true,true,false),Hvd(true),Asp(false),Tcsp(true),Eab(false),Bpsp(false),Tditpsp(false),Amp(true),2010-01-01,None,None,None,None,false,None))))),Some(AgentBusinessPremises(true,Some(List(AgentDetails(Sole Proprietor,None,Some(1970-01-01),Some(entity name),AgentPremises(string,Address(string,string,Some(string),Some(string),GB,Some(AA1 1AA),None),true,Msb(false,false,false,false,false),Hvd(false),Asp(false),Tcsp(false),Eab(true),Bpsp(true),Tditpsp(false),Amp(true),Some(2008-01-01),None),None,false,None,None,None,None,None,None)))))),BankDetails(3,Some(List(BankAccount(Personal account,Personal,true,ukAccount(112233,12345678)), BankAccount(Business account,This business's,false,AccountNumber(12345678)), BankAccount(Another Business account,Another business's,false,IBANNumber(12345678))))),Some(MoneyServiceBusiness(Some(MsbAllDetails(Some(£15k-50k),true,Some(CountriesList(List(GB))),true)),Some(MsbMtDetails(true,Some(123456),IpspServicesDetails(true,Some(List(IpspDetails(name,123456789123456)))),true,Some(12345678963),Some(CountriesList(List(GB))),Some(CountriesList(List(LA, LV))),None)),Some(MsbCeDetailsR7(Some(true),Some(CurrencySourcesR7(Some(MSBBankDetails(true,Some(List(Bank names)))),Some(CurrencyWholesalerDetails(true,Some(List(wholesaler names)))),true)),12345678963,Some(CurrSupplyToCust(List(USD, MNO, PQR))))),None)),Some(Hvd(true,Some(1978-02-15),false,true,Some(40),Some(HvdFromUnseenCustDetails(true,Some(ReceiptMethods(true,true,true,Some(foo))))))),Some(Asp(true,None)),Some(AspOrTcsp(Some(SupervisionDetails(true,Some(SupervisorDetails(Company A,1993-08-25,1999-08-25,false,Ending reason)))),Some(ProfessionalBodyDetails(true,Some(details),Some(ProfessionalBodyDesMember(true,Some(MemberOfProfessionalBody(true,true,false,false,false,false,false,false,false,false,false,false,false,true,Some(test))))))))),Some(TcspAll(true,Some(12345678))),Some(TcspTrustCompFormationAgt(true,true)),Some(EabAll(true,Some(Details),false,None)),Some(EabResdEstAgncy(true,Some(Property Redress Scheme))),Some(Amp(TransactionsAccptOvrThrshld(true,Some(2019-09-19 16:58:06.259Z)),true,60)),Some(List(ResponsiblePersons(Some(NameDetails(PersonName(Some(name),Some(some),Some(surname)),Some(OthrNamesOrAliasesDetails(true,Some(List(Doc)))),Some(PreviousNameDetails(true,Some(PersonName(Some(fname),Some(mname),Some(lname))),Some(1990-02-24),true)))),Some(NationalityDetails(true,Some(IdDetail(Some(UkResident(nino)),None,Some(1970-01-01))),Some(GB),Some(GB))),Some(ContactCommDetails(test@test.com,07000001122,None)),Some(CurrentAddress(AddressWithChangeDate(ccLine 1,ccLine 2,None,None,GB,Some(AA1 1AA),None))),Some(0-6 months),Some(AddressUnderThreeYears(Address(Line 1,Line 2,None,None,GB,Some(BB1 1BB)))),Some(7-12 months),Some(AddressUnderThreeYears(Address(e Line 1,e Line 2,Some(e Line 3),Some(e Line 4),GB,Some(CC1 1CC)))),Some(1-3 years),Some(PositionInBusiness(Some(SoleProprietor(true,true,Some(false),None)),None,None)),Some(RegDetails(false,None,true,Some(0123456789))),true,Some(Some training),true,Some(test),Some(2022-07-07),false,None,Some(false),Some(true),RPExtra(None,None,None,None,None,None,None,None,None)))),AboutYouRelease7(Some(IndividualDetails(fName,None,lName)),true,Some(RolesWithinBusiness(false,false,false,false,false,false,false,true,Some(Agent))),Some(RoleForTheBusiness(false,true,Some(Agent)))),Declaration(true),Some(LettingAgents(Some(true))))
*/
