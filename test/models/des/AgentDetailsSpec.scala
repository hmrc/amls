...
import models.des.tradingpremises.{AgentDetails, AgentPremises, Asp, Bpsp, Eab, Hvd, Msb, Tcsp, Tditpsp, Address => DesAddress}
    ...
    "convert frontend Trading premises into backend model in Amendment flow" in {
      ...
      AgentDetails.convert(feTradingPremises) must be(AgentDetails("Sole Proprietor",None,Some("1970-01-01"),Some("entity name"),
        AgentPremises("string",DesAddress("string","string",Some("string"),Some("string"),"GB",Some("string"),None),true,
          ...
          Tcsp(false),Eab(true),Bpsp(true),Tditpsp(false),Some("2008-01-01"),None,None,None),None,None,None,None))
    }
  ...
  "AgentDetails" must {
    "convert frontend Trading premises into backend model in Amendment flow" in {
      ...
      AgentDetails.convert(feTradingPremises) must be(AgentDetails("Sole Proprietor",None,Some("1970-01-01"),Some("entity name"),
        AgentPremises("string",DesAddress("string","string",Some("string"),Some("string"),"GB",Some("string"),None),true,
          Msb(false,false,false,false,false),Hvd(false),Asp(false),
          Tcsp(false),Eab(true),Bpsp(true),Tditpsp(false),None,None,None,None),Some("2008-01-01"),None,None,None))
    }
