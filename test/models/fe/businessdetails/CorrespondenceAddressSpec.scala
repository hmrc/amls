...
import models.des.aboutthebusiness.{Address => DesAddress, AlternativeAddress}
...
class CorrespondenceAddressSpec extends PlaySpec {
  ...
  "CorrespondenceAddress" must {
    ...
    "convert des model to frontend ATB model" in {
      ...
      CorrespondenceAddress.conv(desModel) must be(
        Some(NonUKCorrespondenceAddress("name", "trade name", "addr1", Some("addr2"), None, None, "UK"))
      )
    }
